...
export const subirImagenTemporal = async (
  usuarioID: string,
  uri: string
): Promise<string> => {
  const blob = await uriToBlob(uri);
  const timestamp = Date.now();
  const tempRef = ref(storage, `temp_validation/${usuarioID}/${timestamp}.jpg`);
 ...
 */
export const subirImagenFinal = async (
  usuarioID: string,
  uri: string
): Promise<string> => {
  const blob = await uriToBlob(uri);
...
export const actualizarFotoPerfil = async (
  ...
  await updateDoc(usuarioRef, { fotoPerfil: foto });
};
